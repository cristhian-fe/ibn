...
var nnManager = new Class({
	...
	{
		var url = 'download.nonumber.nl/extensions.php?j=25';
		if (NNEM_KEY) {
			url += '&k=' + NNEM_KEY;
		}
		nnScripts.loadajax(url, 'nnManager.setExternalData(data.trim())', 'nnManager.setExternalData(0)', '', NNEM_TIMEOUT);
	},
	...
	{
		document.getElement('#show_toggle').setStyle('display', (mode == 'show') ? 'none' : 'inline');
		document.getElement('#hide_toggle').setStyle('display', (mode == 'show') ? 'inline' : 'none');

		table = document.getElement('div#nnem table');
		if (table) {
			if (mode == 'hide') {
				table.addClass('hide_not_installed');
			} else {
				table.removeClass('hide_not_installed');
			}
		}
		this.updateCheckboxes();
	},
	...
	updateCheckboxes: function()
	{
		...
		div.getElements('table tr.not_installed').each(function(tr)
		{
		...
		div.getElements('table.hide_not_installed tr.not_installed').each(function(tr)
		{
